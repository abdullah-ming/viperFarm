...
contract AtpStaking is Ownable {
    ...
    function deposit() public payable {
        ...
        if(msg.value > 0) {
            IWATP(WATP).deposit{value: msg.value}();
            assert(IWATP(WATP).transfer(address(this), msg.value));
    ...
    function safeTransferATP(address to, uint256 value) internal {
    ...
    function withdraw(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            IWATP(WATP).withdraw(_amount);
            safeTransferATP(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accCakePerShare).div(1e12);
